...
function navigateTo(page, extra) {
  ...
  if (pageEfetiva === 'painel')      renderPainelPage();  if (pageEfetiva === 'examesSolicitacoes') renderExamModule('request-management', 'examesSolicitacoesPage');
  if (pageEfetiva === 'examesResultados')   renderExamModule('results-view', 'examesResultadosPage');
  if (pageEfetiva === 'medicacoes')  renderMedicacoesPage();
  if (pageEfetiva === 'medicos')     renderMedicosPage();
  if (pageEfetiva === 'configuracoes') renderConfiguracoesPage();
  if (pageEfetiva === 'consulta')   renderConsultaAtivaPage(consultaAtivaPacienteId);
}
...
function renderSidebar() {
  ...
  const navClinicaVisivel = sess && sessaoEhMedico()
    ...
    : navClinica;
  const navPrincipalVisivel = sess && sessaoEhMedico()
    ? navPrincipal.filter(x => x.page !== 'painel')
    : navPrincipal;
  const navItem = item => {
...
function agendaCardHtml(c) {
  ...
  let acoesAgenda = '';
  if (c.badge === 'agendado' && c.pacienteId) {
